...
void search(const Boidtree &tree, const Boid *self, const Rectangle area, std::vector<Boid> &search_results) {
    if (tree.bounds.no_intersection_with(area)) {
        return;
    }
    ...
    while (true) {
        ...
        if (!ascended) {
            ...
            if (tree.node_has_children(node_index) && new_bound.has_intersection_with(area)) {
